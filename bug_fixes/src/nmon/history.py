...
class HistoryStore:
    ...
    def add_gpu_samples(self, samples: list[GpuSnapshot]) -> None:
        ...
        if self._settings.poll_interval_s > 0:
        ...
        if max_size > 0 and len(self._gpu_snapshots) >= 0.8 * max_size:
            try:
                with DbConnection(self._settings.db_path) as db:
                    if not self.flush_to_db(db, clear_gpu=True, clear_ollama=False):
                        logging.warning("Failed to flush GPU samples to DB")
                        return
            except Exception as e:
    ...
    def add_ollama_sample(self, sample: OllamaSnapshot) -> None:
        ...
        # Flush to DB if we're getting close to max size
        if max_size > 0 and len(self._ollama_snapshots) >= 0.8 * max_size:
            try:
                with DbConnection(self._settings.db_path) as db:
                    if not self.flush_to_db(db, clear_gpu=False, clear_ollama=True):
                        logging.warning("Failed to flush Ollama samples to DB")
                        return
            except Exception as e:
                logging.warning(f"Failed to flush Ollama samples to DB: {e}")
    ...
    def flush_to_db(self, db: DbConnection, clear_gpu: bool = True, clear_ollama: bool = True) -> bool:
        try:
