...
def compute_alert(snapshot: OllamaSnapshot, settings: Settings, now: float) -> Optional[AlertState]:
    ...
    if snapshot.gpu_use_pct < 100 and snapshot.gpu_layers is not None:
        ...
        offload_alert_pct = settings.offload_alert_pct
        min_alert_display_s = settings.min_alert_display_s
        
        if offload_alert_pct is None or not isinstance(offload_alert_pct, (int, float)):
            offload_alert_pct = 90.0
        if min_alert_display_s is None or not isinstance(min_alert_display_s, (int, float)):
            min_alert_display_s = 5.0
            
        if snapshot.gpu_use_pct > offload_alert_pct:
            color = "red"
        else:
            color = "orange"
            
        # Set expiration time
        expires_at = now + min_alert_display_s
